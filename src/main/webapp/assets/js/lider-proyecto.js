...
function renderLeaderProjects(projects) {
  ...
  if (!projects.length) {
    target.innerHTML = `<button class="empty-project-card" type="button" id="empty-new-project"><span>＋</span><strong>No tienes proyectos</strong><b>Crea una nueva iniciativa</b><small>Click para iniciar</small></button>`;
    document.querySelector('#empty-new-project').addEventListener('click', openForm);
    return;
  }

  target.innerHTML = projects.map((project) => `<article class="leader-card" data-leader-project-id="${esc(project.id)}"><div class="window-bar"><span class="dot-red"></span><span class="dot-yellow"></span><span class="dot-green"></span></div><div class="leader-card-body"><div class="card-status">● ACTIVE_BUILD <span>#${esc(project.id)}</span></div><h2>${esc(project.title)}</h2><p>${esc(project.description)}</p><div class="project-mini-data"><span>▷ Plazo: ${esc(project.duration)}</span><span>⌘ Stack: ${esc(project.stack)}</span><span>♟ Equipo: ${esc(project.team)}</span></div><div class="leader-tags">${project.tags.map((tag) => `<span>${esc(tag)}</span>`).join('')}</div><div class="card-actions"><button type="button" class="card-secondary" data-action="metricas">⌚ Métricas del proyecto</button><button type="button" class="card-secondary" data-action="sprints">Sprints</button></div><button type="button" class="card-applicants" data-action="postulantes">♖ Ver postulantes</button></div></article>`).join('') + `<button class="new-initiative-card" type="button" id="grid-new-project"><span>＋</span><strong>Crear nueva iniciativa</strong></button>`;
...
document.querySelectorAll('.stack-tag').forEach((tagButton) => {
  tagButton.addEventListener('click', () => {
    ...
    if (selectedStack.includes(tech)) {
      selectedStack = selectedStack.filter((item) => item !== tech);
    } else {
      selectedStack.push(tech);
    }

    stackHiddenInput.value = selectedStack.join(' · ');
  });
});
...
function resetStackPicker() {
  ...
  document.querySelectorAll('.stack-tag.selected').forEach((tag) => tag.classList.remove('selected'));
}
...
document.querySelector('#leader-form').addEventListener('submit', async (event) => {
  ...
  return; // corta la función aquí, no sigue con el registro
}
document.querySelector('#stack-error').style.display = 'none';
  ...
  const project = {
    ...
    duration: `Entrega: ${data.get('duration')}`,
    team: `${data.get('team')} devs`,
    tags: data.get('stack').split('·').map((tag) => tag.trim()).filter(Boolean)
  };

  try {
    if (window.DevMatchApi.enabled) {
      await window.DevMatchApi.createLeaderProject(project);
    ...
    document.querySelector('#result-vacancies').textContent = project.tags.length;
